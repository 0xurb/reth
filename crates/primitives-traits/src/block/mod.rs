...
use crate::{BlockHeader, SealedHeader};
    ...
    + Into<(Self::Header, Self::Body)>
{
    ...
    /// Sealed full block.
    type SealedBlock: From<(SealedHeader<Self::Header>, Self::Body)>;

    /// Sealed block with senders recovered from transactions.
    type BlockWithSenders: From<(Self, Vec<Address>)>;
    ...
    fn seal_slow(self) -> Self::SealedBlock {
        ...
        Self::SealedBlock::from((SealedHeader::new(header, seal), body))
    }
    ...
    fn seal(self, hash: B256) -> Self::SealedBlock {
        let (header, body) = self.into();
        Self::SealedBlock::from((SealedHeader::new(header, hash), body))
    }
    ...
    fn try_with_senders_unchecked(
        ...
        Ok(Self::BlockWithSenders::from((self, senders)))
    }
    ...
    fn with_recovered_senders(self) -> Option<Self::BlockWithSenders> {
        ...
        Some(Self::BlockWithSenders::from((self, senders)))
    }
